...
**Do not shell out to `python scripts/eval_submission.py` as the default path.** It still calls `utils.call_llm` (Groq/Gemini API). Those free-tier keys do not serve production. Stage 0/1 LLM is Agy. The script also inflated pop_up_talent's cover letter to 92 against an honest 69 (2026-08-17). CR-121 will add `run_stage2_rubric.py` (Agy, CR-112 scorecard) after frozen-floor calibration. Until that ships, score inline against `data/conversion_rubric.md`.
