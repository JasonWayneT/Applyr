...
# A WARN finding with no disposition parks a subphase at WAITING_FOR_HUMAN --
# that is your stop to resolve inline (see the rule immediately below), not Jason's.
...
**WAITING_FOR_HUMAN is the agent's stop, not Jason's (2026-08-30, Jason-directed).** `dispositions.json` requires one of `RESOLVED_EDIT` / `ACCEPTED_AS_CORRECT` / `NOT_APPLICABLE` / `FALSE_POSITIVE` / `HUMAN_ACCEPTED_RISK` per WARN finding before a Stage 2 subphase can complete — but nothing in the code requires that disposition come from Jason specifically, it just requires an accountable judgment call recorded against that exact finding's content. Make that call yourself, in the same session, immediately: if the finding is a real gap (e.g. a term that's genuinely true of Jason and JD-relevant but missing from the draft), fix the document and dispose `RESOLVED_EDIT`; otherwise dispose `ACCEPTED_AS_CORRECT` / `NOT_APPLICABLE` / `FALSE_POSITIVE` with real reasoning — then `--resume` again in the same breath. **Never end a session with a submission sitting at WAITING_FOR_HUMAN** — that pulls Jason into a mid-pipeline decision, which is the opposite of what he wants. Found 2026-08-30: a batch left 2 of 15 submissions stuck this way, one of them with the disposition already written and just never re-run. Jason's review happens once, at the end, reading the finished `Resume.pdf`/`CoverLetter.pdf` if he wants to — not on the disposition ledger mid-flight. `BLOCK`-severity findings are different and should still stop you: those are objective defects (a compile failure, a hard-blocked phrase, unused required evidence) that only `RESOLVED_EDIT` or the rare `HUMAN_ACCEPTED_RISK` can clear — go fix the actual content, don't dispose your way past it.
