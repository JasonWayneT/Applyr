...
**Processing job descriptions today?** Invoke the `generate-submission` skill (canonical `.codex/skills/generate-submission/SKILL.md`; Claude pointer `.claude/skills/generate-submission/SKILL.md`). **CSV jobs go through the CR-119 queue, never straight to `run_submission.py`.** If Jason hands you a CSV file, copy it into `data/inbox/csv/` (don't parse it yourself), then run `python scripts/ingest_csv_queue.py` and `python scripts/run_queue_worker.py --worker <your-harness-id>`. The worker keeps claiming the next batch until nothing left can move (FR-373). `--once` stops after one batch and leaves the rest sitting. If he already used the Pipeline page **Upload CSV**, the file is already in that inbox and ingest has already run; just run the worker. The worker claims its own pack, holds the lease and per-slug lock, and invokes `run_submission.py {slug}` for each job. Don't pre-claim with `queue_claim.py` (that's for inspection only; a pre-claimed pack sits idle while the worker claims a different one). Never run `run_submission.py` by hand on a slug that is in `pipeline_queue`, including `--resume`: a hand run takes no lock, so a worker can run the same folder at the same time. After a Stage 1 paste or writing dispositions, run the worker again; it picks the paused job back up. Same-posting Applied+ or an already-archived pack is already-handled (CR-123): no new queue row, no new `pending_review/` folder, no skip-ledger write; unlocked queue rows go `done`; Stage 0 is `ALREADY_HANDLED`, not PASS-with-flag and not Skip. Pre-apply still reuses. Different-role at the same company still only flags. Close stale mirrors with `python scripts/ingest_csv_queue.py --reconcile-already-handled` (no new CSV). Unknown named tools not in `workExperience.md` do not pause Stage 0 and do not withhold authoring (FR-379 / FR-381). They stay unused on that resume. A Review Center card exists only when that tool is why a job never went out. "I have used this" writes the tool into work experience and does not rerun jobs. Do not fill skill cards to unstick the queue. Requirement-extraction review still pauses. Scout-exported JDs are not queued yet (CR-119 Phase B) and still run direct. CSV/scout JDs land in `data/pending_review/`, not `data/submissions/` (CR-091). Per slug run `python scripts/run_submission.py data/pending_review/{slug}` (or `data/submissions/{slug}` after PASS). That orchestrator is the sole writer of `workflow_state.json` + `stage_receipts/` (CR-076–084): Stage 0 → packet/prompt → WAITING_FOR_LLM → Stage 1 validate → Truth/ATS/HM/Mech/Policy → optional `--finalize`. Use one author paste of `authoring_prompt.md` per WAITING_FOR_LLM. After the documents land, run `python scripts/run_submission.py data/submissions/{slug} --resume`; mechanical Stage 2 is not a substitute for a real qualitative read before disposing `hm.critical_read`. Do **not** Task/Agent-spawn per JD. Do **not** load `agent_context_pack.md`, `workExperience.md`, `master_claims.json`, or full `AGENTS.md` into the Stage 1 author session. Do **not** invoke `conversion-ready-pass` on generate-submission drafts (its own Stage 2 already covers that trigger). Multi-agent batch (`.claude/workflows/generate-submission-batch.js`) is Jason-opt-in only, never the default. Stage 0 Skip is recorded in the `stage0_skips` ledger (URL, then company+title) and the folder moves to `data/archive/skipped/`. Only PASS folders are promoted into `data/submissions/`. Do not call CR-074 workers as the default sequencing path. Not the CR-070 tracker (superseded for generation).
...
**Operator LLM path (CR-121):** Stage 0 extract/classify/evidence and Stage 1 author/repair run on Agy. The worker sets `APPLYR_STAGE0_SUBSCRIPTION_ADAPTER=1`. Groq and Gemini API keys in Settings are leftover `utils.call_llm` cascade. Those keys are free-tier and do not serve production calls. That is expected, not a missing LLM. Do not treat "Groq/Gemini off" as a blocker. Do not set `APPLYR_STAGE0_CLOUD_LLM=1` on a live worker pack (tests may pin it). `eval_submission.py` still calls that cascade and is not the scoring path. After PASS, a missing named tool is unused and does not pause. Employer names, section-header fragments, and methodology nouns are chrome (CR-124). A required product such as Dynamics or Delta Lake can stay in the reason list and is not claimed. It does not withhold the job (CR-125). Agy scorecards (`run_stage2_rubric.py`) stay off-default until AC-464. When Stage 2 is complete and Stage 3 is ready, the worker runs `--finalize` itself and the job lands in the app as Backlog for review (FR-371). Backlog means the queue finished. It does not mean the resume and letter are safe to send. That still takes a read against work experience (CR-126). A save that does not complete pauses as `finalize_failed` and is not claimed again.

**Active threads, docs, and current state:** live in `docs/spec/08-implementation/` and `docs/spec/05-change-requests/`. CR-125 (decide now, correct later) is the current queue rule. CR-124 (chrome vs product names), CR-122 (unknown tools do not pause; the per-tool inbox is superseded by CR-125), CR-121 (conversion feasibility + Agy rubric), CR-123 (already-applied queue dedup), CR-120 (evidence-first authoring, switch off), CR-119 (CSV queue), CR-112/114/115. Each is a self-contained handoff doc with checkbox-tracked stories — open it, find the first unchecked item, start there. Treat those docs as the current source of truth for status, not this file — this section previously carried a full per-CR status table that went stale faster than it could be kept in sync here.
...
| `.agents/skills/submission-no-ai-slop/SKILL.md` | **CR-098 judgment-only no-ai-slop check.** Mechanical patterns live in `submission_linter.py`. Cursor and Antigravity load this path; Claude Code has a pointer under `.claude/skills/`. First draft uses `authoring_prompt.md`, not this skill. | Stage 2 / conversion-ready-pass Pass 3 / "reads as AI" |
...
| `data/authoring_defect_historical_baseline.json` | **CR-099 informational historical baseline** (gitignored). Human-reviewed structured archived evidence; never counts as live CR-097 data. Written by `scripts/import_historical_defects.py`. | When auditing pre-launch authoring history |
...
| `data/candidate_preferences.json` | Filter preferences: no solo/founding PM roles, no 0-to-1. Fit pass/skip floors live in `data/fit_rubric_calibration.json` (CR-093); the old `min_fit_score` here no longer applies — do not resurrect it. | When evaluating job fit |
...
- AI/ML model training, fine-tuning, or ML engineering (approved AI-usage claims like ACC-167, ACC-179, ACC-401, and ACC-120 within its attribution limits are fine)
...
Engineering, DBA, DevOps, Customer Experience (CX), Customer Support, Sales, Account Management, Legal, InfoSec, Product Marketing, Executive/Presidential Leadership, Upgrades (per ACC-104). Do NOT add teams from a JD that aren't on this list. Design / UX is not on this list. Jason has not worked with a design team.
...
| Closed-lost / Closed Lost | Lost subscriptions / lost subscription opportunities |
...
**Resume.pdf MUST be exactly 1 page.** This has been violated by external tools before — every resume came out 2 pages because of the extra sections + bullet bloat described above. Page count is mechanically gated by `verify_submission.py`'s `_pdf_page_count` (run via `run_submission.py`'s Mech stage; see Required Verification below). Do not submit, and do not tell Jason something is done, without that check passing.
...
# A WARN finding with no disposition parks a subphase at NEEDS_DISPOSITION -- that
# means retry it inline yourself right now (see the rule immediately below), not a
# stop for Jason. (Renamed from WAITING_FOR_HUMAN under CR-107 -- that name read as
# an instruction to stop and wait, which is the opposite of what should happen here.)
...
**NEEDS_DISPOSITION is retry behavior, not human review behavior (renamed from WAITING_FOR_HUMAN under CR-107, 2026-08-30, Jason-directed).** The old name was found to actively cause the bug it was meant to prevent — an agent reading a status called "WAITING_FOR_HUMAN" reasonably concludes it should stop and wait for a human, which is backwards: this is a WARN finding the agent is expected to resolve itself, in the same breath, and continue. `dispositions.json` requires one of `RESOLVED_EDIT` / `ACCEPTED_AS_CORRECT` / `NOT_APPLICABLE` / `FALSE_POSITIVE` / `HUMAN_ACCEPTED_RISK` per WARN finding before a Stage 2 subphase can complete — nothing in the code requires that disposition come from Jason specifically, it just requires an accountable judgment call recorded against that exact finding's content. Make that call yourself, immediately: if the finding is a real gap (e.g. a term that's genuinely true of Jason and JD-relevant but missing from the draft), fix the document and dispose `RESOLVED_EDIT`; otherwise dispose `ACCEPTED_AS_CORRECT` / `NOT_APPLICABLE` / `FALSE_POSITIVE` with real reasoning — then `--resume` again in the same breath, exactly like retrying after fixing any other failed check. **Never end a session with a submission sitting at NEEDS_DISPOSITION** — that pulls Jason into a mid-pipeline decision, which is the opposite of what he wants. Found 2026-08-30: a batch left 2 of 15 submissions stuck this way, one of them with the disposition already written and just never re-run — the old name is judged to have contributed to that, hence the rename rather than just restating the rule again. Jason's review happens once, at the end, reading the finished `Resume.pdf`/`CoverLetter.pdf` if he wants to — not on the disposition ledger mid-flight. `BLOCK`-severity findings are different and should still stop you: those are objective defects (a compile failure, a hard-blocked phrase, unused required evidence) that only `RESOLVED_EDIT` or the rare `HUMAN_ACCEPTED_RISK` can clear — go fix the actual content, don't dispose your way past it.
...
- Not an AI/ML engineer or model owner (AI product PM roles are in scope; a "shipped AI features" requirement is a soft gap, not an exclusion)
