...
- Stage 1 validation failure writes workflow `FAILED`. The worker maps that to `paused`, releases the lease, and never auto-promotes it. `build_stage1_repair_prompt.py` writes the repair prompt and leaves the row paused. Requeue happens only after `run_stage1_repair.py` returns valid artifacts and writes them. Timeout or a failed repair call stays paused `FAILED` and counts as no-progress.
