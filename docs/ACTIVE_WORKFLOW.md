...
| 4b | **CSV drop-folder ingest (CR-119)** — put `applyr_jobs*.csv` in `data/inbox/csv/` (or use Review Center **Upload CSV**, which writes to the same inbox and runs ingest). Then `python scripts/ingest_csv_queue.py` if you dropped the file by hand. Run the pack with `python scripts/run_queue_worker.py --worker <id>` (the worker claims; do not pre-claim). Each job still uses `python scripts/run_submission.py {slug} --resume` (bare slug). `--resume` and `run_submission.py` are unchanged. | queued rows in `pipeline_queue`; Review Center pipeline panel |
