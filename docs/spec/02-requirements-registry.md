...
| `AC-443` | acceptance | P0 | accepted | Review Center shows live counts for all six states. Current leases show worker ID and lease age in minutes. Stuck items (expired lease, or last stage receipt older than `STUCK_STALE_MINUTES` = 120) appear in a distinct list. Each item links to the existing job detail view. Quarantine count is visible; navigating to it shows source file, line, and error code per row. Existing job list is not replaced or hidden. Panel is read-only. | `FR-345` | CR-119 |

### Data Traceability (DATA-006)

| ID | Type | Priority | Status | Requirement | Acceptance | Source |
|---|---|---|---|---|---|---|
| `DATA-006` | data | P0 | accepted | Queue state (status, locked_by, lease_expires_at, fencing_token, queued_at, claimed_at, nullable `networking_contacts_raw`), file-ingest ledger (sha256 uniqueness key, filename for display, ingested_at, row_count, quarantine_count), and quarantine records live in the gitignored `jobagent.sqlite` (ADR-001). `data/inbox/csv/` and per-slug lock files at `data/queue_locks/{slug}.lock` are gitignored / inside the data tree. No queue state, lease data, quarantine detail, `networking_contacts_raw`, or JD/PII from CSV rows enters tracked files, git commits, test fixtures, or log files. | — | CR-119 |
...
| `SEC-007` | security | P0 | accepted | Ingest file writes are restricted to paths inside the repo data tree: `data/inbox/csv/` (inbox, archive, quarantine), `data/pending_review/`, and `jobagent.sqlite`. No write occurs outside these expected paths. JD text and Networking Contacts values from CSV rows do not enter tracked fixtures, git commits, test reports, or log files. (CR-119) |
