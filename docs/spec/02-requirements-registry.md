...
| `FR-322` | functional | P1 | draft | Rubric completion scores are hash-bound, role-tagged, and fail-closed on disagreement. Independent blind scoring is required only inside a 3-point band around CONVERT-READY floors. Clear-margin scores stay one qualitative read. No majority vote and no picking the higher score | `AC-420` | CR-112 |
| `AC-420` | acceptance | P1 | draft | Design-only this pass. Next story: stale document hash blocks floors; current-hash 70 vs 68 blocks; 70 implementer plus 71 blind on the same hash completes; resume 78 needs no blind row; helpers call no LLM | `FR-322` | CR-112 |
