...
2. [ ] Build a Stage 0-only subscription adapter pinned to tested claudexor. Set explicit profile, timeout, call ceiling, schema validation, redaction policy, and cache identity. No metered API fallback. Exhaustion or invalid output yields review. Adapter module and mocked tests exist locally (2026-09-17); independent QA remains. A quoted live smoke swallowed CLI flags and Codex ran after Cursor dropped; the adapter now fail-closes on harness substitution and non-readonly access. Production switch off. Cursor route still unverified.
...
5. [ ] Connect uncertain evidence batches to the adapter only after the matcher and harness pass replay. Preserve the existing hard-gate safeguards and checkpoint/resume contract. Intentionally not wired (2026-09-17): cascade still uses the existing provider chain.
6. [ ] Surface the basis, source excerpt, uncertainty, and correction action for useful Stage 0 decisions in Review Center. Reject generic traits such as critical thinking as named-skill cards. Named-tool filter landed in `6941020`. Requirement and evidence excerpt already render. Decision basis and uncertainty are not yet populated from Stage 0.
7. [ ] Inventory all Stage 0 hosted call sites, run 5-10 JD schema smoke and a timed 30-JD replay, set budgets from evidence, and only then enable the replacement path. Record rollback and telemetry. Stages 1-3 are out of scope. Hosted-call inventory written; schema smoke and 30-JD replay have not passed. Production switch off.
