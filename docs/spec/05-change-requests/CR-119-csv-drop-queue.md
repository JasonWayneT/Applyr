...
19. **Phase A pipeline panel has no claim/release/reclaim.** Counts, leases, stuck list, quarantine detail, links into the existing job view. CSV upload into `data/inbox/csv/` is in scope (addendum FR-347).
...
| `in_progress` | `paused` | `run_submission.py` returns `WAITING_FOR_LLM` after at most one in-lease `run_stage1_author.py` attempt (prompt present, `Resume.md` absent), or returns `NEEDS_DISPOSITION` / `WAITING_FOR_INPUT` / `FAILED`. The worker releases the lease (`locked_by` and `lease_expires_at` cleared). A paused job does not hold a pack slot. Missing prompt or a partial Stage 1 set (AC-448) pauses without authoring. |
...
| `AC-458` | Worker still `in_progress`: `WAITING_FOR_LLM` plus `authoring_prompt.md` and no `Resume.md` runs `run_stage1_author.py` once, then `run_submission.py --resume`. Missing prompt or existing `Resume.md` does not author. Author failure still maps to `paused`. Partial Stage 1 files stay paused (AC-448). | FR-344 |
| `AC-459` | A paused Stage 1 `FAILED` row whose packet is over token budget and has no `Resume.md` is claimed once. `--resume` rebuilds the packet with current leftover-trim. `stage1_budget_retry.json` blocks a second loop. `FAILED` with a resume still never auto-promotes. | FR-344 |
...
- **Mutating queue actions in Review Center** (claim / release / reclaim buttons) — still out of scope. CSV upload into the existing inbox is in scope (FR-347).
...
## Addendum 2026-09-18 (Jason)

Two follow-ups on the Implemented CR. Same CR, not a new number.

### A. NEEDS_DISPOSITION promotion loop (bug)

`check_stage1_ready` stays true after Stage 1, so a job paused at `NEEDS_DISPOSITION` was promoted on every later claim, re-ran Stage 2 (including Mech PDF compile), paused again, and looped.

- `WAITING_FOR_LLM` still promotes when `contracts.check_stage1_ready(folder)` is True.
- `NEEDS_DISPOSITION` promotes only when `reviews/dispositions.json` has an mtime later than `pipeline_queue.paused_at`.
- Additive column `paused_at TEXT` (nullable), set when status becomes `paused`. Migration `026_add_pipeline_queue_paused_at.sql` plus Python `ensure_schema`.

`AC-449`: a `NEEDS_DISPOSITION` job with Stage 1 files present and no newer `dispositions.json` stays `paused` across claims.

### B. UI upload lands in the inbox (feature)

Any CSV, harness or UI, goes to `data/inbox/csv/`. `POST /api/pipeline-queue/upload` accepts `.csv` only, enforces a 10 MiB cap, writes a server-chosen filename (never the client path) only under `data/inbox/csv/` (SEC-007), then runs the same ingest as `ingest_csv_queue.py` and returns `{ queued, duplicate, quarantined }`. Review Center pipeline panel gains an upload control. Delete unused `BulkUploadForm.tsx` rather than leave a second parser. POST remains token-gated. No claim/release/reclaim buttons.

`FR-346` / `AC-449` (promotion). `FR-347` / `AC-450` (upload).

OQ-TL-3 is amended: the panel stays without claim/release/reclaim; CSV upload into the existing inbox is now in scope.
