...
| `FR-316` / `AC-413` | CR-112 | Cost eligibility, no free→paid fallback, receipt-aware cost-pause status, pause to paste, and authenticated backend start/resume/status/finalize operator with a Review Center UI | `scripts/cost_eligibility.py`, `scripts/build_stage0_fit_gate.py`, `scripts/stage0_evidence_cascade.py`, `scripts/contracts.py`, `scripts/run_submission.py`, `server/services/runSubmissionRunner.ts`, `server/routes/runSubmission.ts`, `src/components/WorkflowOperator.tsx`, `src/lib/workflowOperator.ts`, `scripts/test_cr112_story71.py`, `tests/unit/runSubmissionRunner.test.ts`, `tests/unit/runSubmissionRoute.test.ts` | in_progress |
