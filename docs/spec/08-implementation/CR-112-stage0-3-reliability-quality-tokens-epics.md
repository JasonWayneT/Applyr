...
**Landed this afternoon:** Epic 1 and Epic 2 (pending review checkboxes). SupplyHouse drafts accepted as-is (do not re-author).

**Implementation paused (2026-09-10, Jason):** preserve the tree, stop further implementation, review **Epic 1 only** and separately from Epics 2–6. A landed-code summary does not establish that Stage 0–3 is ready. Checkboxes stay open.

**Cut mid-Epic 3 (2026-09-10, context-window handoff):** finished in the follow-up session. Epics 3–6 code landed 2026-09-10, pending review. SupplyHouse still not re-authored.
...
**Definition of Done:** invented sequential IDs cannot attach a judgment to a requirement by list position; a packet that had to drop `claim_constraints` cannot be `ready`; every already-shipped artifact affected by the constraint-wipe bug (SupplyHouse confirmed, others unknown until audited) is detected and has an explicit, human-reviewed recovery decision recorded — fixing the generator does not by itself repair what it already produced. Existing hash-suffix / unique-ordinal fallbacks remain. Antigravity files are revised, not deleted.
...
**Status:** [ ] code landed 2026-09-10; pending review. `test_invented_sequential_id_resolves_within_bucket` inverted to `test_invented_sequential_id_is_rejected`; shuffle case added. Position map removed from `_resolve_item_id`. Isolated cascade suite green (24 tests).
...
**Status:** [ ] code landed 2026-09-10; pending review. Wipe block removed. `TestClaimConstraintsBudget.test_over_budget_does_not_wipe_claim_constraints_to_ready` green. supplyhouse not rebuilt.
...
**Status:** [ ] CHANGELOG [DRAFT] 2026-09-10 written; pending review. Registry `FR-296`–`FR-298` / `AC-393`–`AC-395`.

### Story 1.4 — Detect and recover other artifacts affected by the constraint-wipe bug

Story 1.2 fixes the code going forward and explicitly does **not** touch `data/submissions/supplyhouse/authoring_packet.json`, which already shipped with `packet_status=ready` and `claim_constraints={}` (F3). Fixing the generator does not repair packets it already produced. Without a separate detection/recovery pass, SupplyHouse's live packet — and any other folder with the same silent drop — stays broken indefinitely with nothing flagging it.

**Files:**
- New: a detection script under `scripts/` (e.g. `scripts/audit_packet_integrity.py`) — read-only, scans every `data/submissions/*/authoring_packet.json` for `packet_status=ready` with `claim_constraints` empty/missing while `evidence_map` or `soft_gaps` is non-empty (the same shape as the SupplyHouse defect).
- Modify: this file, to record the audit's findings once run.

**Acceptance:**
- Detection is read-only and non-destructive: it reports affected folders (slug, packet_status, constraint count, estimated_tokens at wipe time), it does not rewrite anything.
- Recovery path is explicit and requires a human decision per affected folder, not an automatic rebuild: options are (a) re-run packet assembly under the Story 1.2 fix and re-author Stage 1 if the resulting packet differs materially, or (b) mark the folder `NEEDS_DISPOSITION`-equivalent with a recorded reason if the existing draft is judged acceptable despite the gap (e.g. attribution was independently verified some other way). Neither path is silently automatic.
- Run once against the current 7 live folders as part of this story; report the result in this file (even if the only affected folder turns out to be supplyhouse).
- Does not rebuild or finalize any submission as a side effect of running the detector.

**Dependencies:** Story 1.2 (need the fixed generator before recovery-rebuilding anything).
**Status:** [ ] detector landed 2026-09-10; pending review. Live scan recorded below. No rebuild.
...
**Recovery decision (human, not automatic):** SupplyHouse is the only affected live folder. **2026-09-10, Jason:** do **not** rebuild the packet and do **not** re-author Resume.md / CoverLetter.md. The end-product drafts were already corrected in a Claude session. Recorded as option (b): accept the existing documents despite the empty `claim_constraints` fence. The packet remains historically wiped and the detector will still flag it; that is the known gap, not an open rewrite. Other six folders: no action.
...
**Status:** [ ] code landed 2026-09-10; pending review. Stage 0 default is `run_submission.py data/pending_review/{slug}`; no WE rescore. Stage 1 paste is `authoring_prompt.md` only. Stage 2 `--resume` is mechanical; qualitative read required before `hm.critical_read`.
...
**Status:** [ ] code landed 2026-09-10; pending review. `reviewPrompt` uses packet excerpts + `claim_constraints`; does not load WE. Header/whenToUse say never-default. Cap 3 unchanged.
...
**Status:** [ ] code landed 2026-09-10; pending review. Root `AGENTS.md` only.
...
**Status:** [ ] code landed 2026-09-10; pending review. WARN only (`truth.provenance.extra.<id>`). Prefix does not clear. Tests in `scripts/test_cr112_epic3.py`. Live scan recorded below. No live rewrites.
...
**Status:** [ ] code landed 2026-09-10; pending review. `omitted_reasons` on evidence_map rows; full scores in `evidence_selection_trace.json`. Pearl-like fixture emits `top2_cutoff`. Author prompt does not contain candidate scores. Packet suite re-run green after the additive field.
...
**Status:** [ ] code landed 2026-09-10; pending review. `scripts/report_evidence_swaps.py`. Label mapping covered in `test_cr112_epic3.py`.
...
**Status:** [ ] code landed 2026-09-10; pending review. Fingerprint and nights-and-weekends fixtures have empty `claim_ids` and no ACC-103-ROADMAP.
...
**Status:** [ ] code landed 2026-09-10; pending review. Unhandled name FAIL. `case_stale_hash` fixture SKIPPED (programmatic STATE-003 covers it). Runner 13/13 passed, SKIPPED excluded from pass count.
...
**Status:** [ ] code landed 2026-09-10; pending review. STATE-004 incomplete False / chained receipts True without mocking the oracle. Catalog column `Adversarial runner` marks FIT-*/DOC-002/004/TRUTH-002/003/FINAL-* as `not this runner`. Payloads archived under `docs/spec/archive/adversarial-payloads/`.
...
**Status:** [ ] code landed 2026-09-10; pending review. STATE-001 named substring + negative control; DOC-003 linter-direct + negative control. STATE-002 still asserts skip lock via status (not a bare WorkflowError success). Audit list is in `run_adversarial_pressure_test.py`.
...
**Status:** [ ] code landed 2026-09-10; pending review. Frozen sanitized set in `tests/fixtures/cr112_eval/` (5 fictional JDs). Runtime copy `data/eval/cr112/` (gitignored). Paid calls = 0. Production sqlite refused. Investigation §5 baseline filled.
...
**Status:** [ ] code landed 2026-09-10; pending review. `run_events.jsonl` on eval folders. Columns kept separate: `prompt_meta_estimated_tokens`, `call_llm_invocations`, `harness_spawn_count`. Baseline: all three paid/spawn counters 0; prompt-meta estimate 742 (bytes÷4, not a tokenizer count).
...
## Review checkpoint (implementation paused)

**2026-09-10, Jason:** preserve the work, pause further implementation, review Epic 1 separately from later epics. Nothing in a session summary alone establishes that Stage 0–3 is ready.

Checkboxes stay open. Do not self-mark done. Do not start a new CR.

Settled:
- SupplyHouse: accept existing Claude-corrected drafts; do not rebuild packet; detector still flags.
- Stage 2 qualitative-review contract is written into generate-submission + AGENTS.
- Extra-packet is WARN, not a hard block, until Jason reviews the 4/7 live false-positive-or-real mix.
- Gerund reporter is advisory. No LW rule.
- Eval paid calls stay 0 unless Jason sets `APPLYR_CR112_PAID_BUDGET`.

## First story for the next session

Independent review of CR-112 stories (QA/security/EM). Not a new investigation. Not a new CR.
