...
**Status:** [x] accepted locally on `cr112-story22` (2026-09-11). File is force-added despite `.claude/` gitignore. `reviewPrompt` forbids `workExperience.md`. `whenToUse` is never-default. Cap 3 + `allowLargeBatch` unchanged. Claude Workflow() primitives kept; portable rewrite deferred. No push/merge.
