...
**Status:** [ ] code landed 2026-09-10; pending review. `omitted_reasons` on
...
**Status:** [x] retained on `cr112-epic4`; independent review pending/local commit after verify. Unhandled name FAIL closed.
...
**Status:** [x] retained on `cr112-epic4`; catalog `not this runner` + payload archive present; pending local commit after review. STATE-004 incomplete
...
**Status:** [x] retained on `cr112-epic4`; pending local commit after review. STATE-001 named
...
success). Audit list is in `run_adversarial_pressure_test.py`. Fixture
`case_forbidden_punctuation` is smoke; programmatic DOC-003 is authoritative.
