...
### TL;DR — read this, then go (updated 2026-09-20, Claude Sonnet 5)

**Progress: 2 of 5 done and finalized (rentana, healthstream — both Stage 3 `COMPLETE`, both PDFs sent to Jason, both `done` in `pipeline_queue`). 1 correctly closed without finalizing (binance — real Mandarin gap, Jason confirmed, leave it alone). 1 resolved to a legitimate SKIP, not a bug (casper_studios — see 9s below). 37 still queued, untouched.**

**Jason confirmed live this session:** finalize healthstream now (done — see 9s), and fix the casper requeue bug now rather than deferring it (done — see 9s).

**Immediate next steps, in order:**
1. Work the 37-job backlog toward the 5-job target (3 more needed: rentana + healthstream done; binance and casper_studios were legitimate closes, not counted): `python scripts/run_queue_worker.py --worker <your-id> --size 1 --once` repeatedly (start with size 1, not the default 8, until you've confirmed the fixes below are holding on a couple of fresh jobs). **Do not bypass Agy or hand-author — see the hard rule above.**
2. Each new job gets 9p/9q/9r/9s's protection automatically on its first Stage 0/Stage 1 pass, so should need less hand-holding than binance/healthstream/casper did.
