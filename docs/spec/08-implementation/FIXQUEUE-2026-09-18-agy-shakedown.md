...
### TL;DR — read this, then go (updated 2026-09-20 04:42 UTC, Claude Sonnet 5)

**Progress: 4 of 5 reached `ready_to_finalize` (Stage 2 COMPLETE). Rentana, healthstream, and indigo are fully finalized (Stage 3 `COMPLETE`, PDFs sent). Tuniform just reached `ready_to_finalize` (9v below, PDFs sent, awaiting Jason's finalize go-ahead — do not `--finalize` without it).** Obie reached Stage 2 Mech but is genuinely blocked on the rubric floor pending Jason's call (obie — see 9u, do NOT dispose past `mech.rubric_floor.resume` without his real sign-off; does not count toward the 5). Binance correctly closed without finalizing (real Mandarin gap). Casper_studios resolved to a legitimate SKIP. 34 still queued, untouched.

**Immediate next steps, in order:**
1. Check whether Jason has responded on tuniform (finalize?) and obie (send as-is despite the real 65/100 resume score, or pass). Tuniform: if approved, `python scripts/run_submission.py data/submissions/tuniform --finalize` — that's the 5th job, target met. Obie: if he says send, write `HUMAN_ACCEPTED_RISK` (role `human_reviewer`) as the `mech.rubric_floor.resume` disposition; if he says pass, leave its queue row as-is (`NEEDS_DISPOSITION`, paused), do not force it further.
2. If the 5-job target is already met once tuniform is confirmed, the weekend goal is done — check with Jason on whether to keep working the 34-job backlog or stop. If continuing: `python scripts/run_queue_worker.py --worker <your-id> --size 1 --once` repeatedly (still size 1, not the default 8). **Do not bypass Agy or hand-author — see the hard rule above.**
3. Every new job gets 9p/9q/9r/9s/9u/9v's protection automatically on its first Stage 0/Stage 1 pass. Indigo (9t), obie (9u), and tuniform (9v) are the three most complete worked examples of the disposition/rubric-scoring machinery — read all three before your first NEEDS_DISPOSITION pause if this is new to you, especially the rubric-floor/boundary-band sections in 9u and 9v.
