...
### TL;DR — read this, then go (updated 2026-09-20 03:20 UTC, Claude Sonnet 5)

**Progress: 3 of 5 done. Rentana and healthstream are finalized (Stage 3 `COMPLETE`, `done` in `pipeline_queue`, PDFs sent to Jason). Indigo just reached `ready_to_finalize` (Stage 2 COMPLETE, PDFs sent to Jason, awaiting his finalize go-ahead — do not `--finalize` it without that).** 1 correctly closed without finalizing (binance — real Mandarin gap, Jason confirmed, leave it alone). 1 resolved to a legitimate SKIP, not a bug (casper_studios — see 9s below). 36 still queued, untouched.

**Immediate next steps, in order:**
1. Check whether Jason has responded on indigo (finalize / edit / pass). If approved: `python scripts/run_submission.py data/submissions/indigo --finalize`.
2. Work the 36-job backlog toward the 5-job target (2 more needed after indigo counts): `python scripts/run_queue_worker.py --worker <your-id> --size 1 --once` repeatedly (start with size 1). **Do not bypass Agy or hand-author — see the hard rule above.**
3. Each new job gets 9p/9q/9r/9s's protection automatically on its first Stage 0/Stage 1 pass, so should need less hand-holding than binance/healthstream/casper did. Indigo (9t below) still needed 6 rounds of hand-fixing after the fresh author call and is the most complete worked example yet of the disposition/rubric-scoring machinery — read it before your first NEEDS_DISPOSITION pause if this is new to you.
4. **New capability, use it going forward:** `python scripts/run_stage1_author.py <folder> [--wall-seconds N]` is now a real, tested script (not an ad-hoc command) for the fresh isolated Stage 1 author call — reads `authoring_prompt.md`, sends it over stdin as stream-json (avoids the Windows "Argument list too long" limit that `--print` hits on a 45-49KB prompt), extracts and writes all three required artifacts (Resume.md, CoverLetter.md, claim_provenance.json — all three required, unlike repair). Use this instead of hand-rolling the binance/healthstream-style one-off command. `scripts/run_stage1_repair.py` is still the separate, existing tool for repairing an already-drafted job.
