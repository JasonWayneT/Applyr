...
### TL;DR — read this, then go (updated 2026-09-19 end of session, Claude Sonnet 5)

**Progress: 2 of 5 done (rentana, healthstream — both `ready_to_finalize`/`COMPLETE`, both PDFs already sent to Jason). 1 correctly closed without finalizing (binance — real Mandarin gap, Jason confirmed, leave it alone). 1 waiting on Jason (casper_studios — Review Center questions). 38 still queued, untouched.**

**Immediate next steps, in order:**
1. Check whether Jason has responded on healthstream (finalize / edit / pass) and on casper_studios' Review Center questions. If finalize is approved for healthstream: `python scripts/run_submission.py healthstream --finalize` (writes the real jobs DB — confirm with Jason first if not already explicit).
2. If casper_studios' questions are answered: `python scripts/queue_claim.py requeue --slug casper_studios --reason "Review Center answers complete" --worker <your-id>` then run the worker. Watch for the Stage 0 evidence-omission pattern (item 5/9k already improved this, not guaranteed fixed).
3. Then work the 38-job backlog toward the 5-job target: `python scripts/run_queue_worker.py --worker <your-id> --size 1 --once` repeatedly (start with size 1, not the default 8, until you've confirmed the fixes below are holding on a couple of fresh jobs). **Do not bypass Agy or hand-author — see the hard rule above.**
...
**Fixes landed this session (all committed, commit range `424f975`..`2b442ef`, all with tests passing):** 9m (repair citation-lookup bug), 9n (employment-logistics + LR-026 "epic" false positives), 9p (required-non-English-language hard gate), 9q (Core Competencies wrongly needing citation), 9r (travel-percentage line wrongly needing a claim bridge). Full detail on each is in the log below if you need to understand *why*, not just *what*.

**Known real bugs NOT yet fixed (both logged in detail below, both worth fixing before running the full 38-job backlog unattended):**
