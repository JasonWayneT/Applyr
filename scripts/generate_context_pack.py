...
CLAUDE.md, SKILL.md, data/workExperience.md, and data/conversion_rubric.md
...
Why a *generated* file, not a hand-edited one: CLAUDE.md and SKILL.md
...
- CLAUDE.md: drop two named top-level sections that are pure engineering/
...
# Exact ## heading text (without the "## " prefix) to drop from CLAUDE.md.
CLAUDE_MD_EXCLUDE_SECTIONS = frozenset(
    {
...
def build_manifest() -> dict:
    ...
    return {
        ...
        "sources": {
            os.path.relpath(CLAUDE_MD, REPO_ROOT).replace(os.sep, "/"): _sha256(CLAUDE_MD),
...
def generate_pack() -> tuple[str, dict]:
    with open(CLAUDE_MD, encoding="utf-8") as f:
        claude_trimmed = filter_sections(f.read(), CLAUDE_MD_EXCLUDE_SECTIONS)
    ...
    parts = [
        ...
        "ONE lean file instead of cold-reading CLAUDE.md + SKILL.md + "
...
def main() -> None:
    ...
    source_chars = sum(
        os.path.getsize(p)
        for p in (CLAUDE_MD, SKILL_MD, WORK_EXPERIENCE_MD, CONVERSION_RUBRIC_MD, MASTER_CLAIMS_JSON)
    )
